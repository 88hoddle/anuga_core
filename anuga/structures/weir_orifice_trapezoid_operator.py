...
def weir_orifice_trapezoid_function(
    ...
    if blockage >= 1.0:
        ...
        Q_inlet_unsubmerged = 1.7*bf*((2*width+depth*(z1+z2))/2)*driving_energy**1.50 # Flow based on Inlet Ctrl Inlet Unsubmerged (weir flow equation)
        Q_inlet_submerged = 0.8*bf*anuga.g**0.5*(0.5*depth*(2*width+depth*(z1+z2)))*driving_energy**0.5  # Flow based on Inlet Ctrl Inlet Submerged (orifice equation)
    ...
    if delta_total_energy < driving_energy:
        ...
        if outlet_enquiry_depth > depth:        # The Outlet is Submerged
            ...
            flow_area=bf*width*depth+0.5*(z1+z2)*(depth)**2  # Cross sectional area of flow in the culvert
            perimeter=bf*width + ((depth)**2+(z1*(depth))**2)**0.5 + ((depth)**2+(z2*(depth))**2)**0.5
...
##calculate normal depth based on culverts slope, i can't work out how to get culvert_slope, so for now just use critical depth instead as we do in boyd_box and boyd_pipe         
            ...
            if outlet_culvert_depth > depth:
                outlet_culvert_depth=depth
                flow_area=bf*width*depth+0.5*(z1+z2)*(depth)**2
                perimeter=bf*width + ((depth)**2+(z1*(depth))**2)**0.5 + ((depth)**2+(z2*(depth))**2)**0.5
    ...
    return Q*barrels, barrel_velocity, outlet_culvert_depth, flow_area*barrels, case
